...
public class Lexer {
    ...
    public Lexer(ArrayList<String> Code, ArrayList ErrorList){
        this.Code = Code;
        this.ErrorList = ErrorList;
    }


    public Lexer(ArrayList<String> tokens, ArrayList<String> lineCount , ArrayList ErrorList){
        //this.Code = Code;
        this.ErrorList = ErrorList;
        this.lineCount = lineCount;

        for(int i =0; i< tokens.size(); i++)
            analyseToken(tokens.get(i));
        ...
        for (int i=0; i < lineCount.size(); i++){
            if(i == index){
                for(int j =0; j < size; j++){
                    newLineCount.add(lineCount.get(index));
                }
            }else{
    ...
    private ArrayList<String> scanWord(String word) {
        ...
        /*if(wordChanged == true && word.isEmpty() == false){
            output.add(word);
        }*/
        if( (word.isEmpty() == false)){ //&& (output.isEmpty() == true) ){
            output.add(word);
        }
    ...
    private void analyseToken(String word) {
        ...
        if (checkTokens.contains(word) ) {
        ...
        else if( (((word.charAt(0) >= '0') && (word.charAt(0) <= '9')) || (word.charAt(0) >= '-')) && (numberPattern.matcher(String.valueOf(word)).matches()) ){ //integers (numberPattern.matcher(word.substring(1)).matches())
            ...
            for(int i=1 ; i < word.length(); i++){
                if (((word.charAt(i) >= 'a') && (word.charAt(i)  <= 'z')) || ((word.charAt(i) >= '0') && (word.charAt(i)  <= '9'))) {
                    ...
                    lexerErrorFound("user-defined variable found to be illegal (cannot contain uppercase)", word); //String.valueOf(word.charAt(i)));
                    break;
                }
                else{
    ...
    private  void lexerErrorFound(String error, String symbol){
        ...
        String finalError = "Lexical error: " + "\n" + word + "\n" ;
        for(int i =0; i < info.size(); i++){
            finalError = finalError + info.get(i)+ "\n";
        }
    ...
    public ArrayList<String> getLineCount() {
        return lineCount;
    }

    public void setLineCount(ArrayList<String> lineCount) {
        this.lineCount = lineCount;
    }
}
